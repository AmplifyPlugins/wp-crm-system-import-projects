...
if( isset( $_FILES[ 'import_projects' ] ) && isset( $_POST[ 'wpcrm_system_import_projects_nonce' ] ) ){
	if( wp_verify_nonce( $_POST[ 'wpcrm_system_import_projects_nonce' ], 'wpcrm-system-import-projects-nonce' ) ){
		...
		if( !in_array( $file_type, $csv_types ) ){
			$errors[] = __( 'File not allowed, please use a CSV file.', 'wp-crm-system-import-projects' );
		}
		if( $file_size > return_bytes( ini_get( 'upload_max_filesize' ) ) ){
			$errors[] = __( 'File size must be less than', 'wp-crm-system-import-projects' ) . ini_get( 'upload_max_filesize' );
		}	
		if( empty( $errors ) == true ){
			...
			<div id="message" class="updated">
				<p><strong><?php _e( 'Projects uploaded. ', 'wp-crm-system-import-projects' ); echo $count_added; _e( ' added. ', 'wp-crm-system-import-projects' ); echo $count_skipped; _e( ' skipped.', 'wp-crm-system-import-projects' ); ?> </strong></p>
			</div>
		<?php } else { ?>
...
<div class="wrap">
	<div>
		<h2><?php _e( 'Import Projects From CSV', 'wp-crm-system-import-projects' ); ?></h2>
				...
				<form id="wpcrm_import_projects" name="wpcrm_import_projects" method="post" action="" enctype="multipart/form-data">
					<tr>
						...
						<td>
							...
							$link = sprintf( wp_kses( __( 'Important: Please make sure your CSV file is in the <a href="%s">correct format</a>. If it is out of order, your fields will not be imported correctly. ', 'wp-crm-system-import-projects' ), array(  'a' => array( 'href' => array() ) ) ), esc_url( $url ) );
							echo $link; ?>
						</td>
					</tr>
				</form>
				<form id="wpcrm_export_projects" name="wpcrm_export_projects" method="post" action="">
					<tr>
						<td>
							...
							<input type="submit" name="export_projects" value="<?php _e( 'Export Projects', 'wp-crm-system-import-projects' ); ?>" />
						</td>
						<td></td>
					</tr>
				</form>
